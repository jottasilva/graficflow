...
import { validateAndLogPermissions } from "../shared/permissions";
...
type OrderDto = {
  ...
  paymentStatus?: "PENDING" | "PARTIAL" | "PAID" | "OVERDUE" | "REFUNDED" | "CANCELED" | null;
  total: number | string | null;
...
function mapOrder(order: OrderDto, clients: Client[], products: Product[], sectors: Sector[]): Order {
  ...
  const items: OrderItem[] = (order.order_items ?? []).map((item) => {
    ...
    const total = numeric(item.total ?? unitPrice * qty);
    return {
      id: item.id,
      productId: item.productId ?? "",
      productName: itemProduct?.name ?? item.description ?? "",
      description: item.description ?? itemProduct?.name ?? "",
      quantity: qty,
      unitPrice,
      total,
      sector: itemSector?.name ?? "",
  ...
  return {
    ...
    paidAmount: numeric(order.paidAmount),
...
function mapUser(user: UserDto): UserAccount {
  ...
  return {
    ...
    role: user.role,
    permissions: validateAndLogPermissions(user.permissions ?? [], `mapUser(${user.id})`),
...
export const graphflowApi = {
  ...
  async createPayment(
    input: Omit<PaymentTransactionRecord, "id" | "tenantId" | "feeAmount" | "netAmount" | "createdAt" | "updatedAt"> & {
      feeAmount?: number;
    },
  ) {
  ...
  async loadWorkspace(): Promise<WorkspaceData> {
    const [clients, users, sectors, products, inventory, finance, payments, files, notifications] = await Promise.all([
      this.listClients(),
      this.listUsers(),
      this.listSectors(),
      this.listProducts(),
    ...
    return { clients, users, products, inventory, machines, sectors, orders, quotes, finance, payments, files, notifications };
  },
  ...
  async createUser(input: Omit<UserAccount, "id" | "tenantId" | "status"> & { password?: string; status?: UserAccount["status"] }) {
    ...
    const sanitizedPermissions = validateAndLogPermissions(input.permissions ?? [], `createUser(${input.email})`);
  ...
  async updateUser(id: string, input: Partial<Omit<UserAccount, "id" | "tenantId">>) {
    ...
    if ("permissions" in sanitizedInput && sanitizedInput.permissions !== undefined) {
      sanitizedInput.permissions = validateAndLogPermissions(sanitizedInput.permissions, `updateUser(${id})`);
    }
  ...
  async updateSector(id: string, input: Partial<Pick<Sector, "name" | "capacity" | "sla" | "lead">> & { kanbanOrder?: number }): Promise<Sector> {
